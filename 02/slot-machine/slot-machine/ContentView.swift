    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .center, spacing: 50) {
                HStack {
                    Image("fire")
                        .resizable()
                        .scaledToFit()
                        .shadow(color: .orange, radius: 1, x: 1, y: 3)
                    Text("Slot Machine")
                        .font(.system(size: 30))
            ...
            .alert(item: $showingAlert) {
                ...
                switch alert {
                case .success:
                    return Alert(title: Text("Yahhh! You won!"), message: Text("Born with the charm"), dismissButton: .cancel(Text("Play again!")))
                case .failure:
                    return Alert(title: Text("Ooooops!"), message: Text("Better luck next time"), dismissButton: .cancel(Text("Try again")))
                }
            }
        }
    }
